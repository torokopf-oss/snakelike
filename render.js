...
// Новая функция: создаёт какашку за указанным хвостом с учётом направления
...
function drawGame(t, now) {
    ...
    for (let i = 0; i <= maxY(); i++) {
        ctx.beginPath();
        ctx.moveTo(0, i * gs);
        ctx.lineTo(viewCells * gs, i * gs);
        ctx.stroke();
    }
    for (let i = 0; i <= viewCells; i++) {
        ctx.beginPath();
        ctx.moveTo(i * gs, 0);
        ctx.lineTo(i * gs, maxY() * gs);
        ctx.stroke();
    }
    ...
    if (!worldDiscovered && snake.length >= 30) {
        ...
        ctx.beginPath();
        ctx.moveTo(wallX, 0);
        ctx.lineTo(wallX, maxY() * gs);
        ctx.stroke();
        ctx.restore();
    }

    // Нижняя граница перед третьей фазой (5+ малышей)
    if (!worldDiscoveredDown && babySnakes.length >= CONFIG.babyThresholdForThirdPhase) {
        ...
        ctx.beginPath();
        ctx.moveTo(0, wallY);
        ctx.lineTo(CONFIG.viewWidth * gs, wallY);
        ctx.stroke();
        ctx.restore();
    }
    ...
    if (warningActive && spawnSide !== -1) {
        const pulse = Math.sin(warningPulse) * 0.4 + 0.6;
        const gw = 20;
        ctx.save();
        ctx.fillStyle = `rgba(255,80,0,${pulse * 0.8})`;
        ctx.shadowBlur = 15;
    ...
    for (let b = 0; b < babySnakes.length; b++)
        drawSnake(babySnakes[b], babyPrevSnakes[b], '#f1c40f', '#f39c12', 0, 0, gs, t);

    if (egg) {
        const ex = egg.x * gs, ey = egg.y * gs;
        ctx.fillStyle = '#ecf0f1';
        ctx.beginPath();
        ctx.ellipse(ex + gs / 2, ey + gs / 2, gs / 2 - 2, gs / 2 - 4, 0, 0, Math.PI * 2);
        ctx.fill();
    }
    ...
    for (const p of poops) {
        ...
        ctx.beginPath();
        ctx.ellipse(px + gs / 2, py + gs / 2, gs / 2 - 2, gs / 3 - 1, 0, 0, Math.PI * 2);
        ctx.fill();
    }
    ...
    for (let i = 0; i < foods.length; i++) {
        ...
        const px = (prev.x + (f.x - prev.x) * t) * gs;
        const py = (prev.y + (f.y - prev.y) * t) * gs;
        ctx.beginPath();
        ctx.arc(px + gs / 2, py + gs / 2, gs / 2 - 2, 0, Math.PI * 2);
        ctx.fill();
    }

    if (pill) {
        const px = pill.x * gs, py = pill.y * gs;
        const cx = px + gs / 2, cy = py + gs / 2;
        const s = 5, l = gs / 2 - 2;
        ctx.fillStyle = '#fff';
        ctx.fillRect(px + 1, py + 1, gs - 2, gs - 2);
        ctx.fillStyle = '#e74c3c';
        ctx.fillRect(cx - s / 2, cy - l, s, l * 2);
        ctx.fillRect(cx - l, cy - s / 2, l * 2, s);
    }

    ctx.fillStyle = '#e74c3c';
    for (let i = 0; i < vultures.length; i++) {
        const v = vultures[i], prev = prevVultures[i] || v;
        const px = (prev.x + (v.x - prev.x) * t) * gs;
        const py = (prev.y + (v.y - prev.y) * t) * gs;
...
// Отрисовка сброшенных сегментов хвоста
